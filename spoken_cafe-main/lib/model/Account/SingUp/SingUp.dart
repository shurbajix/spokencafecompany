...
class _SignupState extends ConsumerState<Signup> {
  ...
  bool _obscureText = true;
  bool isLoading = false;
  List<bool> isChecked = [false, false]; // [Teacher, Student]
  final GoogleSignIn _googleSignIn = GoogleSignIn();
  ...
  // Future<void> _navigateBasedOnRole(User user, String role, bool isVerified) async {
  //   if (!mounted) return;
  //   setState(() => isLoading = true);

  //   try {
  //     print('Navigating for user: ${user.uid}, role: $role, isVerified: $isVerified');

  //     // Navigate to Login page for all users
  //     if (mounted) {
...
Future<void> _navigateBasedOnRole(User user, String role, bool isVerified) async {
  ...
  try {
    if (role == 'teacher') {
      // Teachers go to the verification screen
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (_) => const VerfiedScreen()),
      );
    } else {
      // Students go straight into the app
      Navigator.pushReplacement(
        context,
  ...
  Future<void> _signUpWithGoogle() async {
    ...
    try {
      ...
      if (user != null) {
        String role = isChecked[0] ? 'teacher' : 'student';
        bool isVerified = role != 'teacher';
        print('Signing up with Google: role=$role, isVerified=$isVerified');

        String? uploadedImage = _imageFile != null ? await _uploadImage(_imageFile!) : null;
        ...
        await _navigateBasedOnRole(user, role, isVerified);
      }
    } catch (e) {
  ...
  Future<void> _signUpWithEmail() async {
    ...
    try {
      String role = isChecked[0] ? 'teacher' : 'student';
      bool isVerified = role != 'teacher';
      print('Signing up with email: role=$role, isVerified=$isVerified');
      ...
      print('Email sign-up error: $errorMessage');
                        ...
                        text: TextSpan(
                          ...
                          children: [
                            TextSpan(
                              text: 'Login',
                              style: const TextStyle(
                                  color: Color(0xff1B1212), fontWeight: FontWeight.bold),
                              recognizer: TapGestureRecognizer()
